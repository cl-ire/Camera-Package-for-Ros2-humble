...
class MovementControl(Node):
    ...
    def listener_callback(self, Position):
        ...
        self.time1        = Position.data[7]
        self.time2        = Position.data[8]
        
        
        use_data, old_time = movement_control_util.compare_times(Position, self.old_time)

        if use_data:
            drive_time = 1
            self.old_time = movement_control_util.add_seconds_to_time(old_time, drive_time)
